  ...
  callbacks: {
    ...
    async signIn({ user, account, profile }) {
      ...
      if (account?.provider === "google") {
        // Wait a moment for Prisma adapter to create the user
        ...
        // If user doesn't exist yet (first-time OAuth), skip org creation here
        // The user will be created by Prisma adapter, org can be created on first dashboard visit
        if (!dbUser) {
          console.log('[Auth] First-time Google OAuth user, skipping org creation');
          return true;
        }

        // If user exists but doesn't have org, create one
