...
export class CalendarManager {
  ...
  async createEvent(eventData: CreateEventInput): Promise<CalendarEvent> {
    ...
    if (reminders.length > 0) {
      // Validate that orgId is present when creating reminders
      if (!eventData.orgId) {
        throw new CalendarValidationError(
          'Organization ID is required when creating reminders'
        );
      }
  ...
  async updateEvent(
    ...
    if (updates.reminders) {
      ...
      // Queue each reminder for processing (use orgId from existing event)
      if (!existingEvent.orgId) {
        throw new CalendarValidationError(
          'Organization ID is required when creating reminders'
        );
      }
      // TypeScript narrowing doesn't work across blocks, use non-null assertion
      const orgId = existingEvent.orgId!;
  ...
  async createReminders(eventId: string, reminderTimes: number[]): Promise<void> {
    ...
    if (remindersToCreate.length > 0) {
      // Validate that orgId is present
      if (!event.orgId) {
        throw new CalendarValidationError(
          'Organization ID is required when creating reminders'
        );
