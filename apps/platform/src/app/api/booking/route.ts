...
export async function POST(req: NextRequest) {
  try {
    ...
    if (DEFAULT_USER_ID) {
      ...
      if (!user.orgId) {
        console.error(`[Booking] Default user has no organization: ${DEFAULT_USER_ID}`);
        return NextResponse.json(
          { error: "Booking system configuration error", details: "Default host user must belong to an organization" },
          { status: 500 }
        );
      }
